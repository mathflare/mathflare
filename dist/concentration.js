"use strict";
document.querySelector('#conccalc').addEventListener('click', (event) => {
    event.preventDefault();
    const conc_select = document.querySelector('#conc-select').value;
    const solution = parseFloat(document.querySelector('#concsolution').value);
    const disolved_substance = parseFloat(document.querySelector('#concdsub').value);
    const W_W = (sol, dsub) => {
    ...
    const clearValues = () => {
        document.querySelector('#concsolution').value = '';
        document.querySelector('#concdsub').value = '';
    };
    if (conc_select === 'ww') {
        document.querySelector('#concout').innerHTML = `${W_W(solution, disolved_substance)}`;
        clearValues();
    }
    else if (conc_select === 'wv') {
        document.querySelector('#concout').innerHTML = `${W_V(solution, disolved_substance)}`;
        clearValues();
    }
    else if (conc_select === 'vv') {
        document.querySelector('#concout').innerHTML = `${V_V(solution, disolved_substance)}`;
        clearValues();
    }
});
document.querySelector('#conc-select').addEventListener('change', (event) => {
    event.preventDefault();
    const select = document.querySelector('#conc-select').value;
    const sol_input = document.querySelector('#concsolution');
    const dsub_input = document.querySelector('#concdsub');
    if (select === 'ww') {
