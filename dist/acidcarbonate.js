"use strict";
document.querySelector('#acidcarboncalc').addEventListener('click', (event) => {
    ...
    if (isNaN(z) || y == '' || x == '' || isNaN(v)) {
        ...
        document.querySelector('#acidcarbonout').innerHTML = `<p class="text-warning">All values must be integers</p>`;
        clearValues();
    }
    else if (z > 3 || v > 3) {
        document.querySelector('#acidcarbonout').innerHTML = `<p class="text-warning">The value of z or v cannot be more than 3</p>`;
        clearValues();
    }
    else {
        if (z == v) {
            if (z == 2) {
                ...
                document.querySelector('#acidcarbonout').innerHTML = `${removeOne(result)}`;
                clearValues();
            }
            else {
