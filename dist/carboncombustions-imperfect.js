    ...
    if (product === 'co') {
        ...
        if (bond === 'single') {
            if (isNaN(c)) {
                ...
                const h = 2 * c + 2;
                const o = (h / 2) / 2;
                document.getElementById('output').innerHTML = `<p class="text-success"><span class="text-warning">H</span>${h}<span class="text-warning">C</span>${c} + ${o}<span class="text-warning">O<sub>2</sub></span> &#8594; ${h / 2}<span class="text-warning">H<sub>2</sub>O</span> + ${c}<span class="text-warning">C</span></p>`;
                document.getElementById('carbons').value = "";
            }
        }
        else if (bond === 'double') {
