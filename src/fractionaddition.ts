(<HTMLButtonElement>document.querySelector('#addfractcalc')).addEventListener('click', (event: Event) => {
    ...
    if (isNaN(x) || isNaN(y) || isNaN(a) || isNaN(b)) {
        ...
        (<HTMLParagraphElement>document.querySelector('#addfractout')).innerHTML = `<p class="text-warning">The value of y and b cannot be 0</p>`;
        clearValues();
    } else {
