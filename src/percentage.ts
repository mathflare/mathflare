...
(document.querySelector('#button') as HTMLButtonElement).addEventListener('click', (event: Event) => {
    ...
    if (isNaN(num) || isNaN(den)) {
        ...
        (document.querySelector('#output') as HTMLElement).innerHTML = `<p class="text-success"><sup>${num}</sup>&frasl;<sub>${den}</sub> = ${(num / den) * 100
            }&#37;</p>`;
        (<HTMLInputElement>document.querySelector('#num1')).value = "";
        (<HTMLInputElement>document.querySelector('#den1')).value = "";
    }
...
(document.querySelector('#button2') as HTMLButtonElement).addEventListener('click', (event: Event) => {
    ...
    if (isNaN(per) || isNaN(num2) || num2 == 0) {
        ...
        (document.querySelector('#output2') as HTMLElement).innerHTML = `<p class="text-success">${per}&#37; of ${num2} = ${(per / 100) * num2
            }</p>`;
        (<HTMLInputElement>document.querySelector('#per2')).value = "";
        (<HTMLInputElement>document.querySelector('#num2')).value = "";
    }
