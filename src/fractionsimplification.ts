(<HTMLButtonElement>document.querySelector('#button')).addEventListener('click', (event) => {
    ...
    const simplification: (num: number, den: number) => string = (num: number, den: number) => {
        if (num / den === Math.floor(num / den)) {
            ...
            if (num < 0 && den < 0) return `${-num / MKD}/${-den / MKD}`;
            else if (num > 0 && den > 0) return `${num / MKD}/${den / MKD}`;
            else return `-${(Math.abs(num) / MKD}/${(Math.abs(den) / MKD}`;
        }
    };
    if (isNaN(x) || isNaN(y)) {
        ...
        (<HTMLParagraphElement>document.querySelector('#output')).innerHTML = `${simplification(x, y)}`;
        clearValues();
    }
});