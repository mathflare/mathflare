(<HTMLButtonElement>document.querySelector('#electroncalc')).addEventListener('click', (event: Event) => {
    event.preventDefault();
    const z: number = parseFloat((<HTMLInputElement>document.querySelector('#atomicnum')).value)!;
    const atomStructure: (atomicnum: number) => void = (atomicnum: number) => {
        const Shells: { [key: number]: string } = {
            1: "H: K(1)",
            2: "He: K(2)",
            3: "Li: K(2) L(1)",
            4: "Be: K(2) L(2)",
        ...
        (<HTMLParagraphElement>document.querySelector('#electronout')).innerHTML = `<p class="text-success">${Shells[atomicnum]}</p>`;
    }
    if (isNaN(z)) {
        (<HTMLParagraphElement>document.querySelector('#electronout')).innerHTML = `<p class="text-warning">Please enter a valid value</p>`;
    } else if (z <= 0) {
        (<HTMLParagraphElement>document.querySelector('#electronout')).innerHTML = `<p class="text-warning">The atomic number must be greater than 0</p>`;
    } else if (Math.floor(z) !== z) {
        (<HTMLParagraphElement>document.querySelector('#electronout')).innerHTML = `<p class="text-warning">The atomic number must be an integer</p>`;
    } else if (z > 118) {
        (<HTMLParagraphElement>document.querySelector('#electronout')).innerHTML = `<p class="text-warning">There is no element with an atomiv number of ${z}</p>`;
    } else {
        atomStructure(z);
    }
});
(<HTMLButtonElement>document.querySelector('#clearbtn')).addEventListener('click', (event: Event) => {
    (<HTMLInputElement>document.querySelector('#atomicnum')).value = '';
    (<HTMLParagraphElement>document.querySelector('#electronout')).innerText = '';
});