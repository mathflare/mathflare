...
//X2CO3z + HvY
(<HTMLButtonElement>document.querySelector('#acidcarboncalc')).addEventListener('click', (event: Event) => {
    ...
    if (isNaN(z) || y === '' || x === '' || isNaN(v)) {
        ...
        (<HTMLParagraphElement>document.querySelector('#acidcarbonout')).innerHTML = `<p class="text-warning">All values must be integers</p>`;
    } else if (z > 3 || v > 3) {
        (<HTMLParagraphElement>document.querySelector('#acidcarbonout')).innerHTML = `<p class="text-warning">The value of z or v cannot be more than 3</p>`;
    } else {
        if (z === v) {
            if (z === 2) {// XCO3 + HvY --> XY + H2O + CO2
                ...
                (<HTMLParagraphElement>document.querySelector('#acidcarbonout')).innerHTML = `${removeOne(result)}`;
