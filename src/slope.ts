...
(<HTMLButtonElement>document.querySelector('#slopecalc')).addEventListener('click', (event) => {
    ...
    if (isNaN(x1) || isNaN(y1) || isNaN(x2) || isNaN(y2)) {
        ...
        (<HTMLParagraphElement>document.querySelector('#output')).innerHTML = `<p class="text-warning">The 2 points must have different coordinates</p>`;
        clearValues();
    } else if (x1 === x2) {
        (<HTMLParagraphElement>document.querySelector('#output')).innerHTML = `<p class="text-warning">The slope cannot be calculated between the points (${x1},${y1}) and (${x2},${y2}): they belong to a vertical line</p>`;
        clearValues();
    } else {
        (<HTMLParagraphElement>document.querySelector('#output')).innerHTML = `<p class="text-success">The slope is ${(y2 - y1) / (x2 - x1)}m</p>`;
        clearValues();
    }
});
//copyrights (c) 2021 MathFlare LLC. All rights reserved.