...
(document.querySelector('#button') as HTMLButtonElement).addEventListener('click', (event) => {
    ...
    if (isNaN(a) || isNaN(c)) {
        ...
        (document.querySelector('#output') as HTMLElement).innerHTML = `<p class="text-warning">The equation is a contradiction in R: it has no solutions <br /> X = &#177; ${x}i (&#8764;${x.toFixed(4)}) in C</p>`;
        (<HTMLInputElement>document.querySelector('#a')).value = "";
        (<HTMLInputElement>document.querySelector('#c')).value = "";
    } else if (-c > 0 && a < 0) {
        const x: number = Math.sqrt(-c / -a);
...
(document.querySelector('#button2') as HTMLButtonElement).addEventListener('click', (event) => {
    ...
    if (isNaN(a) || isNaN(b)) {
        ...
        const x1: number = 0;
        const x2: number = - b / a;
        (document.querySelector('#output2') as HTMLElement).innerHTML = `<p class="text-success">x = ${x1} OR x = ${x2}</p>`;
        (<HTMLInputElement>document.querySelector('#a2')).value = "";
        (<HTMLInputElement>document.querySelector('#b2')).value = "";
    }
});
...
(document.querySelector('#button3') as HTMLButtonElement).addEventListener('click', (event) => {
    ...
    if (isNaN(a) || isNaN(b) || isNaN(c)) {
        ...
        if (x == 0) {
            ...
            const x: number = Math.sqrt(c / a);
            (document.querySelector('#output3') as HTMLElement).innerHTML = `<p class="text-warning">The equation is a contradiction in R: it has no solutions <br /> x = &#177; ${x}i (&#8764;${x.toFixed(4)}) in C</p>`;
            (<HTMLInputElement>document.querySelector('#a3')).value = "";
            (<HTMLInputElement>document.querySelector('#b3')).value = "";
            (<HTMLInputElement>document.querySelector('#c3')).value = "";
        } else if (-c > 0 && a < 0) {
        ...
        (document.querySelector('#output3') as HTMLElement).innerHTML = `<p class="text-warning">Discriminant(Δ) = ${D} < 0, The equation is a contradiction in R: it has no solutions <br /> x = ${x1} OR x = ${x2} in C</p>`;
        (<HTMLInputElement>document.querySelector('#a3')).value = "";
        (<HTMLInputElement>document.querySelector('#b3')).value = "";
        (<HTMLInputElement>document.querySelector('#c3')).value = "";
    } else if (D == 0) {
        const x1: number = (- b - Math.sqrt(D)) / (2 * a);
        const x2: number = (- b + Math.sqrt(D)) / (2 * a);
        (document.querySelector('#output3') as HTMLElement).innerHTML = `<p class="text-success">Discriminant(Δ) = 0,  x = ${x1} (double solution)</p>`;
        (<HTMLInputElement>document.querySelector('#a3')).value = "";
        (<HTMLInputElement>document.querySelector('#b3')).value = "";
        (<HTMLInputElement>document.querySelector('#c3')).value = "";
    } else {
        const x1: number = (- b - Math.sqrt(D)) / (2 * a);
        const x2: number = (- b + Math.sqrt(D)) / (2 * a);
        (document.querySelector('#output3') as HTMLElement).innerHTML = `<p class="text-success">Discriminant(Δ) = ${D} > 0,  x = ${x1} OR x = ${x2}</p>`;
        (<HTMLInputElement>document.querySelector('#a3')).value = "";
        (<HTMLInputElement>document.querySelector('#b3')).value = "";
        (<HTMLInputElement>document.querySelector('#c3')).value = "";
    }
