(<HTMLButtonElement>document.querySelector('#reductioncalc')).addEventListener('click', (event: Event) => {
    ...
    if (isNaN(x) || isNaN(y)) {
        ...
        (<HTMLParagraphElement>document.querySelector('#reductionout')).innerHTML = `<p class="text-warning">The value of y cannot be 0</p>`;
        clearValues();
    } else {
